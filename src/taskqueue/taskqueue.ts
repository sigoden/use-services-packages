...
export class Service<A> {
  ...
  async start() {
    ...
    for (const name in this.args.handlers) {
      this.queue.process(name, this.args.handlers[name]);
    }
  }

  public async enqueue<K extends keyof A>(name: K, data: A[K], opts?: Bull.JobOptions): Promise<Bull.Job> {
...
export type HandlerFn = () => Promise<void> | void;
